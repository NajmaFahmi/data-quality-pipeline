...
class AlertDispatcher:
    ...
    WARN → Slack only. CRITICAL → Slack + PagerDuty log.
    """
    ...
    def dispatch(self, result: FreshnessResult) -> None:
        ...
        if result.severity == "WARNING":
            self._send_slack(result, channel="#data-quality-alerts")
            return
    ...
    def _send_slack(self, result: FreshnessResult, channel: str) -> None:
        ...
        text = f"{icon} *Freshness Alert* — `{result.table_name}`\n{result.message}"
        ...
        if self.slack_webhook_url:
            try:
                ...
                if response.status_code == 200:
                    logger.info(f"[SLACK → {channel}] Alert sent: {result.message}")
                else:
